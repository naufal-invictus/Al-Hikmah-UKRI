...
export const ChapterReader = ({ chapter, onBack }) => {
  ...
  // --- FIX FORMATTING TEXT (ROBUST) ---
  const formatText = (text) => {
    if (!text) return "";
    return text
      // Bold: **text** -> strong
      .replace(/\*\*(.*?)\*\*/g, '<strong class="font-bold text-[#333]">$1</strong>')
      // Italic: *text* -> em (Hati-hati jangan menimpa tag HTML yang sudah ada)
      .replace(/(?<!<[^>]*)\*(.*?)\*(?![^<]*>)/g, '<em class="italic text-[#555]">$1</em>');
      // Regex Italic di atas menggunakan negative lookbehind/ahead agar tidak merusak tag HTML jika ada
  };
      ...
      case 'image':
        ...
        return (
          <figure key={idx} className={cn("mb-6 flex flex-col w-full", isLeft ? "md:float-left md:mr-8 md:w-[45%]" : "md:float-right md:ml-8 md:w-[45%]", commonClasses)}>
            ...
            {/* FIX: Caption sekarang support formatting */}
            {block.caption && <figcaption className="text-center text-sm text-[#8A8A8A] mt-2 font-sans-modern italic px-2" dangerouslySetInnerHTML={{ __html: formatText(block.caption) }} />}
          </figure>
        );
      ...
      case 'quote':
        return (
          <div key={idx} className={cn("relative bg-[#FAF9F6] p-6 rounded-2xl my-8 border border-[#EAE0D5] clear-both", commonClasses)}>
            ...
            {/* FIX: Quote Text support formatting */}
        ...
        return (
          <ul key={idx} className={cn("space-y-3 my-6 pl-2", commonClasses)}>
            {block.items.map((item, i) => (
              <li key={i} className="flex items-start gap-3 text-[#4A4A4A] font-sans-modern text-lg">
      ...
      case 'analogy':
        return (
          <div key={idx} className={cn("bg-white border border-[#EAE0D5] p-6 rounded-2xl shadow-sm my-8 relative overflow-hidden clear-both", commonClasses)}>
            <h3 className="text-md font-bold text-[#333] mb-2 flex items-center gap-2 font-sans-modern uppercase tracking-wider text-[#D4AF37]">💡 {block.title}</h3>
      ...
      case 'reflection':
        return (
            <div key={idx} className={cn("bg-[#333] text-[#FAF9F6] p-8 rounded-xl shadow-lg my-10 text-center clear-both", commonClasses)}>
               <h3 className="text-xs font-bold text-[#D4AF37] mb-3 uppercase tracking-[0.2em] font-sans-modern">Renungan</h3>
               {/* FIX: Reflection Text support formatting */}
    ...
    return (
        <div className="min-h-screen flex flex-col items-center justify-center px-6 bg-[#FAF9F6] text-center">
            <FadeContent>
                ...
                <p className="text-[#6B6B6B] mb-2 font-sans-modern">Skor Kuis Anda: <strong className="text-[#D4AF37]">{Math.round(score)}</strong></p>
                <p className="text-sm text-[#8A8A8A] mb-8">Masukkan nama untuk klaim sertifikat.</p>

                <input
                    type="text"
                    placeholder="Nama Lengkap / Kunyah"
                    value={userName}
  ...
  if (viewState === 'certificate') {
      return (
          <div className="min-h-screen flex flex-col items-center justify-center px-4 bg-[#1a1a1a] relative overflow-y-auto py-10">
               ...
               <FadeContent>
                    <div className="text-center text-white/80 mb-6 font-sans-modern text-sm">
                    ...
                    <div className="mt-10 flex gap-4 justify-center">
                        <button onClick={onBack} className="px-6 py-2 rounded-full border border-white/20 text-white/80 hover:bg-white/10 text-sm font-sans-modern transition-all">
    ...
    <div className="min-h-screen bg-[#FAF9F6] flex flex-col">
      <div className="fixed top-0 left-0 w-full z-50 bg-[#FAF9F6]/95 backdrop-blur-sm border-b border-[#EAE0D5] h-16 flex items-center px-4 justify-between">
        ...
        <div className="flex-1 mx-4 max-w-xl">
             <div className="flex justify-between text-[10px] uppercase tracking-widest text-[#8A8A8A] mb-1 font-sans-modern font-bold">
            ...
            >
                {activeSection === 0 && (
                    <div className="text-center mb-8">
                         <span className="inline-block px-3 py-1 mb-4 border border-[#D4AF37]/30 bg-[#D4AF37]/5 rounded-full text-[#D4AF37] text-xs font-bold tracking-widest uppercase font-sans-modern">
                            {chapter.metadata.title}
                        </span>
                        <h1 className="font-serif text-2xl md:text-3xl text-[#333] mb-2 leading-tight">
                            {chapter.content[activeSection].blocks.find(b => b.type === 'heading')?.text}
                        </h1>
                    </div>
                )}

                <div className="flow-root min-h-[50vh]">
                    {chapter.content[activeSection].blocks.map((block, idx) => {
                ...
                {isLastSection && chapter.references && (
                     <div className="bg-[#FAF9F6] border border-[#E5E5E5] rounded-xl p-6 relative mt-12 clear-both">
                        ...
                        {chapter.references.map((ref, idx) => (
                            <li key={idx} className="text-sm text-[#6B6B6B] font-sans-modern italic flex items-start gap-2">
      ...
      <div className="fixed bottom-0 left-0 w-full bg-white border-t border-[#EAE0D5] p-4 z-50">
        <div className="max-w-5xl mx-auto flex justify-between items-center">
            <button onClick={handlePrev} disabled={activeSection === 0} className={cn("flex items-center gap-2 px-4 py-3 rounded-xl font-bold text-sm transition-all font-sans-modern", activeSection === 0 ? "opacity-0 pointer-events-none" : "text-[#6B6B6B] hover:bg-[#FAF9F6] hover:text-[#333]")}>
                <ChevronLeft size={18} />
            </button>
            {isLastSection ? (
